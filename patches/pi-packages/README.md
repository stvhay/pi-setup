...
| `@pi-archimedes/subagent` | 2.1.0 | Add bounded agentic and isolated one-shot execution, independent absolute-duration and sliding-idle limits, provider output-cap evidence, repeated-error termination, bounded parallel/progress rendering, and dual legacy/Pi 0.84 stream parsing. Valid child activity renews idle time; bridged human questions pause it without pausing hard limits. Public `./agents` and `./types` exports expose configured-model resolution and complete ordered child-result evidence, including output-contract labels and child-session correlation. Native v2.1.0 local model/thinking overrides remain authoritative. |
