...
| `@pi-archimedes/footer` | 2.0.1 | Preserve Pi's keyed extension-status contract when the Archimedes custom footer replaces Pi's built-in footer. Newline-delimited entries render on separate sorted, sanitized, width-bounded rows; Caveman and Ponytail render inline on the wide main row when space permits or together on one fallback row. |
