...
- `langfuse-config-env.ts` locks the managed preset to `conversations`, overriding any saved preset while preserving explicit false input/output flags. Its package-only proxy supplies bounded provider and message-event clones without system/developer/tool content and discards handler mutations or replacements, so real provider payloads, local subscription handlers, and persisted messages remain untouched. Unrelated package methods and events still pass through.
...
| `npm:pi-langfuse@1.5.7` | 1.5.7 | Preconfigured + public composition | Package entrypoint autoload is disabled. Before calling its public entrypoint, `langfuse-config-env.ts` sets the managed privacy ceiling used by runtime config and `/langfuse-status`, then proxies package registration of provider-request and message-bearing output events. Generation records use sanitized telemetry clones; real provider requests, local subscription alias/restore handlers, persistence, and exact downstream normalization remain unchanged. Commands use portable select/input/notifications. Agent-os provisions `pi-langfuse/config.json` before launch, preventing its blocking startup setup flow; other RPC deployments must do the same or defer setup until ready. |
