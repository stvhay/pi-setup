...
- `langfuse-config-env.ts` establishes an at-most-`conversations` privacy ceiling before composing the installed `pi-langfuse` public entrypoint between two ordered `message_end` handlers. Tool I/O, system prompts, and cwd are always disabled; explicit stricter input/output settings remain disabled. Subscription generations reach upstream with the zero-priced telemetry alias, while later handlers and session persistence receive the original runtime model ID.
...
| `langfuse-config-env.ts` | Public composition adapter | Reads pre-provisioned config, enforces the managed at-most-`conversations` ceiling before package registration, keeps startup notification non-blocking, and surrounds upstream telemetry with a transient subscription alias that is removed before persistence. |
