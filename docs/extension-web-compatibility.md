...
| --- | --- | --- | --- |
| `npm:pi-archimedes@1.8.3` | 1.8.3 + version-locked session-ID patch | Adapted / partial | Package entrypoint autoload is disabled and `archimedes.ts` calls its public entrypoint through the installed package resolver. It captures upstream ask/subagent tool definitions, preserves their public behavior, adds strict portable question cardinality/custom input and optional delegated output contracts, then delegates execution back to the captured tools. Normal TUI retains Archimedes components, commands, lazy tools, child ask socket, accessible custom picker, and nonempty Other handling. RPC uses portable ask dialogs with equivalent selected/custom result fields plus text todo/subagent widgets; custom `/archimedes` settings and `/agents` manager, component header/footer/editor, and exact terminal-input behavior remain unavailable there. The temporary subagent patch exposes Pi's existing ephemeral session UUID as `childSessionId` without changing `--no-session`. Package evidence: public manifests and entrypoints for `pi-archimedes` and `@pi-archimedes/{core,footer,diff,subagent,ask,image-paste,todo,notify}`. |
