...
| `npm:pi-langfuse@1.5.9` | 1.5.9 + version-locked logical-session patch | Preconfigured + public composition | Package entrypoint autoload is disabled. Before calling its public entrypoint, `langfuse-config-env.ts` sets managed capture values, then composes subscription alias/restore handlers without proxying package events. Commands use portable select/input/notifications. Agent-os provisions `pi-langfuse/config.json` before launch, preventing its blocking startup setup flow; other RPC deployments must do the same or defer setup until ready. The temporary package patch prefers public `SessionManager.getSessionId()` so ephemeral roots retain exact logical identity; downstream normalization leaves raw Langfuse records unchanged. |
