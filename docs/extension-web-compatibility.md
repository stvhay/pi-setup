...
- `langfuse-config-env.ts` composes the installed `pi-langfuse` public entrypoint between two ordered `message_end` handlers. Subscription generations reach upstream with the zero-priced telemetry alias, while later handlers and session persistence receive the original runtime model ID.
...
| `langfuse-config-env.ts` | Public composition adapter | Reads pre-provisioned config, keeps startup notification non-blocking, and surrounds upstream telemetry with a transient subscription alias that is removed before persistence. |
