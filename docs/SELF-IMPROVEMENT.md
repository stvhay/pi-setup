...
Canary effects use `claims.jsonl` as one durable claim/dispatch/settle state machine. Claim and settlement hold short private locks; adapter dispatch runs outside them with an explicit `claimId` token. Existing work/run adapters accept that token as `claimContext`, preserve it under invocation `provenance` and result artifacts, and resolve the exact active durable claim before execution, including receipt, grant, policy, target, action, bounded-effect identity, and split evidence contract; they never infer ownership from process-global state. Terminal states are `dispatched`, `failed`, `uncertain`, and `revoked`; unresolved claims fail closed and never retry automatically. This guarantees at-most-one dispatch attempt. Exactly-once external effects require the adapter to use `claimId` as an idempotency key or equivalent transaction boundary.
