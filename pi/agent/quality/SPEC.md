...
Canary application uses one append-only `claims.jsonl` state machine. `claim` holds the private `fcntl` lock only while reserving one receipt/grant allowance and returns an explicit token containing `claimId`, receipt, allowance, policy/grant, target, action, and bounded effects. `dispatch(packet, claimToken)` runs after that lock is released; the token is passed explicitly, and no process-global reentrancy marker or effect-wide lock is used. Existing work/run adapters accept this token as `claimContext` and preserve it under invocation `provenance`; they validate its deterministic claim identity and grant binding before execution rather than inferring ownership from process state. `settle(claimToken, result)` reacquires the short private lock and appends exactly one terminal transition from `claimed` to `dispatched`, `failed`, `uncertain`, or `revoked`. Repeated settlement returns the existing terminal result without another row or dispatch.

A pre-dispatch crash, timeout, adapter exception, or uncertain settlement is `uncertain` and blocks retry; a hard process kill can leave `claimed`, which is also unresolved and fail-closed. Explicit unknown, critical, drift, expiry, missing-proof, or stop-rule results are `revoked` and trigger best-effort durable grant revocation. Claims provide at-most-one dispatch attempt, not exactly-once external effect. Exactly-once effect requires an adapter idempotency key or transaction using `claimId`.
...
| `apply(receipt_id, ...)` | Reload current policy and a persisted receipt; record disabled/observe state or execute one canary packet through an injected existing dispatch adapter after grant, target, policy, proof, budget, and effect revalidation. Canary adapters receive an explicit claim token and must not infer ownership from process state. |
| `validate_claim_token(value, directory=None, require_active=False)` | Validate the bounded claim context carried across adapters, including deterministic claim/allowance identity and receipt, grant, policy, target, action, and effect bindings; an adapter may also require an exact active durable claim. |
...
| `settle(claim_token, result, ...)` | Idempotently append one terminal claim transition under a short private lock; malformed or ambiguous proof fails closed. |
...
| INV-15 | One canary receipt or grant allowance yields at most one debit and one dispatch attempt. Claims and settlement are atomic under short private `fcntl` locks; dispatch runs outside locks; claimed, uncertain, or revoked claims fail closed. Dispatch requires exact action/effects, target fingerprint, required proof, current policy/grant/budget, and a bounded result; critical failure, drift, expiry, missing proof, unknown result, or stop-rule breach revokes the grant. Explicit run adapters preserve and validate claim context in invocation provenance; exactly-once external effect requires adapter idempotency or transaction semantics. | `claims.jsonl`, explicit claim tokens, validated `claimContext`, double pre-dispatch revalidation, idempotent settlement, local revocation guard, existing run authority check, and canary apply tests. |
...
- Work/run adapters carry canary ownership only through explicit `provenance.claimContext`; they validate claim identity and grant binding before execution and never read a process-global reentrancy or ownership marker.
...
| INV-15, FAIL-13 | `tests/test_quality.py::test_inv15_canary_revalidates_grant_target_and_dispatches_once`, `test_inv15_concurrent_canary_apply_consumes_one_allowance`, `test_inv15_concurrent_same_grant_claims_reserve_one_allowance`, `test_inv15_claim_dispatch_settle_passes_explicit_token_and_releases_lock`, `test_inv15_settlement_is_idempotent_after_terminal_state`, `test_fail13_settlement_rejects_forged_claim_context`, `test_fail13_settlement_downgrades_unknown_dispatched_state_to_uncertain`, and `test_fail13_dispatch_exception_settles_uncertain_and_blocks_retry`; existing grant/run authority checks cover the downstream adapter guard. |
