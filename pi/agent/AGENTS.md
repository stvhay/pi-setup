...
- Run `agnt route --access repository`, then unnamed `subagent` with routed fields; repository access is agentic. Self-contained cold packets use `--access self-contained` and one-shot mode. Use `artifact` for deferred full results; `status-only` for deterministic checks; `inline` when content is needed now; `pass-no-findings` only when explicitly requested.
