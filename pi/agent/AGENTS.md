...
Outside that exact initial scope, branch/worktree deletion, local merge, and deployment require explicit approval. Do not push, merge outside the guarded integration above, deploy outside the verified non-production boundary above, force/reset/clean, delete branches or beads, rewrite history, change Beads remotes/history, or install hooks without explicit approval. Remote deletion and history rewrite remain explicitly gated.
