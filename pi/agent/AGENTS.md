...
- Before code changes in a repository with `.beads/`, use `bd prime` and `bd ready` for context, then start interactive work with `agnt direct start <id> [--claim]`. This validates and shows the Bead, claims only with the explicit flag when needed, and links the current session; it does not create run bundles, runners, or worktrees. At interactive closeout, run `agnt improve outcome <id> <success|partial|failure|unclear>` to guarantee both link and final outcome.
