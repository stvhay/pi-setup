...
- `agnt approvals resolve DECISION_ID --outcome approved|answered|rejected|cancelled|timed-out [--answer TEXT] [--structured-answer] [--selected-option TEXT ...] [--custom-input TEXT]`
  - Records the human outcome and public-safe UI resolver kind in Beads, never private Pi session identifiers. Answered questions may persist predefined selections and typed input separately while retaining a readable answer summary; `--structured-answer` preserves an explicit empty multi-selection. Structured question fields cannot approve an action. Approved/answered decisions close the decision bead; rejected/cancelled/timed-out decisions keep visible blockers.
