...
For interactive delegation, run `agnt route`, then call the Archimedes `subagent` tool with `agent` omitted and the selected target as `model`. Use its `tasks` array for parallel peers. The tracked observer generates one payload-free `invocationId` per child at tool start, reuses it in the completed projection and local metric, and records unnamed-peer usage under `<git-root>/.pi/metrics/invocations/` without storing prompt or response bodies. Parallel indexes are metadata, not identity. Named-profile metrics are skipped because Archimedes does not expose their final provider.
