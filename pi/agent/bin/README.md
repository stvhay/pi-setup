...
- `agnt route --task TASK [--risk low|medium|high] [--budget cheap|balanced|quality] [--context-tokens N] [--modality text|image|audio|video] [--local-ok] [--monthly-paid-spend USD] [--ignore-history]`
  ...
  - For `review`, emits the approved risk-specific `reviewPolicyTargets` fanout and a deterministic monthly spend state: Terra at high thinking by default, Kimi K2.7 Code for medium-risk diversity, and Opus 5 for high-risk independent review. It counts OpenRouter plus catalog venues marked `billingClass: metered`, while excluding subscription-backed GPT opportunity cost. It uses `AGNT_REVIEW_PAID_SPEND_USD` as an operator floor, accepts an authoritative `--monthly-paid-spend` override, and keeps only subscription-backed Terra at the `$18` reserve and `$20` hard-cap thresholds. K3 is escalation-only.
...
- `agnt improve scan [--since ISO] [--limit N] [--max-traces N] [--recheck] [--dry-run] [--json]`
  - Reads up to `--max-traces` traces (default 500) from the bounded Langfuse window and writes a schema-2 private packet under `~/.pi/improvement/`; `--dry-run` writes nothing. The safe command summary remains schema 1, review decisions remain schema 1 with policy `v1`, and historical schema-1 packets remain reviewable. `traceDiscovery` reports the cap, API total when valid, scanned/attributable/unattributed lower bounds, completeness, and continuation state (`max-traces` or `non-advancing-page` when truncated). Selected sessions remain capped at 20 traces and 500 observations per trace with capture-gap markers. Private features report `executionOutcome`, `apparentOutcome`, `finalOutcome`, and `finalOutcomeSource`; latest root execution is used, while delegated output quality stays separate. Additive `errorTaxonomy` keeps count-only raw and unclassified-raw error health separate from payload-free classified signals by expected/recovered/provider/infrastructure/agent/unknown class, source, actionable state, and independent outcome-blocking state; unknown is retained rather than guessed. The exact `pi-langfuse-1.5.7-dual-null-dual-26` TOOL fingerprint produces null input/output byte aggregates and `payloadByteMetadata.toolIo` provenance; missing nonfingerprint metadata nulls only its affected side. Non-null payload-byte values are bounded-preview estimates, not raw payload sizes. Raw Langfuse records remain unchanged, and no tool payload is copied into the packet. Projected outcomes and payload-free tool-error signals join through the private session ID.
