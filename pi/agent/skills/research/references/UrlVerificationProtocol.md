...
---------------------------------------------------------------
  ...
  Research peers/models can HALLUCINATE URLs - NEVER trust them blindly
...
Research peers/models can HALLUCINATE URLs that look plausible but don't exist. This includes:
...
1. **Verify with curl/content fetch** - Actually fetch the URL and confirm it returns content (not 404, 403, or error)
...
# Step 2: If 200, verify content with curl/content fetch
curl -L "URL" | head -c 4000  # confirm this article exists and matches the cited point
...
|------------|--------------|
| URL verified via curl/content fetch returns actual content | URL from research agent without verification |
