...
Adversarial analysis using parallel Pi peer deployment. Breaks arguments into atomic components, attacks from multiple expert perspectives, synthesizes findings, and produces the strongest counter-argument.
...
Launch 4-32 specialized peer passes with unique personalities to examine BOTH strengths AND weaknesses from their perspective. For normal work, use one representative from each family. For high-stakes work, scale up toward the full roster.
...
Wait for all peers to complete. Compile their outputs.
...
[ ] PHASE 2: Parallel Analysis
  [ ] Prepare 4-32 peer prompts with unique personalities
  [ ] Launch peers in parallel (batch into rounds if needed)
  [ ] Each peer: engineer, architect, pentester, or intern type
  [ ] Each peer examines BOTH strengths AND weaknesses
