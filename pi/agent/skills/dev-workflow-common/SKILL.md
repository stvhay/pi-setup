...
- For successful direct work, run `agnt work direct-closeout <id> --outcome success --reason "<reason>"`; it records explicit success through existing improvement logic and verifies matching readback before Git or Beads work. Use `agnt improve outcome <id> <partial|failure|unclear>` separately for non-success or manual/repair recording. Direct closeout creates the second local commit for shared portable Beads state: explicit export, target closeout parity, and all legitimate `.beads/issues.jsonl` rows. It refuses tracked non-Beads changes and never stages or commits them.
