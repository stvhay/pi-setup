...
Decide whether deployment and patch-state behavior preserves rollback and exact installed-runtime invariants.
...
One-shot worker cannot retrieve repository files. Bounded actual caller, loader, and runtime contract evidence, tests, and output schema are embedded below; no raw transcript is included. `deploy` is caller, `patch_state` is loader, and `runtime.py` contents are runtime contract.

## Constraints

Review only B1–B3. Return at most two Critical/Important findings and at most 6,000 characters.
