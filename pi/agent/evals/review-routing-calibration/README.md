...
For each packet and repetition, send one `subagent` tool call containing two tasks in manifest model order. Use the packet-scoped `taskPrefix` when present (currently both deployment packets use `v6` after their exact-state contracts changed); otherwise use the top-level prefix. Build each task as:
