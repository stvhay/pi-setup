...
For each packet and repetition, send one `subagent` tool call containing two tasks in manifest model order. Use the packet-scoped `taskPrefix` when present (currently both deployment packets use `v7` after their boundary evidence changed); otherwise use the top-level prefix. Build each task as:
