...
`agent/extensions/beads-footer.ts` adds one keyed status without replacing the active footer. It lists every canonical `in_progress` Bead as a newline-delimited `<id> P<priority> <title>` entry in priority/ID order and uses the active theme's accent/bold styling for the Bead linked to the current Pi session. Pi's built-in footer renders the keyed status natively; the version-locked Archimedes footer patch renders each Bead entry on its own sorted, sanitized, width-bounded row. Archimedes places the exact `caveman` and `ponytail` statuses on its wide main row when they fit, otherwise together on one fallback row; other statuses retain dedicated rows. The status refreshes on session startup/reload and after tools that can change Beads; empty, malformed, or failed reads clear it. This data remains presentation-only and is not added to model context.
